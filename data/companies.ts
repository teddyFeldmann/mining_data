import type { Company } from "./interfaces";

export const Companies = {
  RioTinto: { name: "Rio Tinto" },
  BHP: { name: "BHP" },
  JECO: { name: "JECO" },
  MPMaterials: { name: "MP Materials" },
  LithiumAmericas: { name: "Lithium Americas" },
  BaotouSteelRareEarthGroup: { name: "Baotou Steel Rare Earth Group" },
  LynasRareEarths: { name: "Lynas Rare Earths" },
  Inalum: { name: "Inalum" },
  FreeportMcMoRan: { name: "Freeport-McMoRan" },
  NevadaGoldMines: { name: "Nevada Gold Mines" },
  NavoiMiningAndMetallurgyCombinat: {
    name: "Navoi Mining & Metallurgy Combinat",
  },
  GoldFields: { name: "Gold Fields" },
  TianqiLithium: { name: "Tianqi Lithium" },
  Albemarle: { name: "Albemarle" },
  SQM: { name: "SQM" },
  Vale: { name: "Vale" },
  Nornickel: { name: "Nornickel" },
  PowerMetallic: { name: "Power Metallic" },
  AngloAmerican: { name: "Anglo American" },
  ZijinMining: { name: "Zijin Mining" },
  DRCGovernment: { name: "DRC Government" },
  IvanhoeMines: { name: "Ivanhoe Mines" },
  Gecamines: { name: "Gecamines" },
  CrystalRiver: { name: "Crystal River Global Limited" },
  GenorahResources: { name: "Genorah Resources (Pty) Limited" },
  CodelcoMitsuiJV: { name: "Codelco-Mitsui (Becrux JV)" },
  MitsubishiCorporation: { name: "Mitsubishi Corporation" },
  JapaneseMitsuiJV: { name: "Japanese consortium (led by Mitsui)" },
  Glencore: { name: "Glencore" },
  MitsubishiDevelopment: { name: "Mitsubishi Development" },
  Itochu: { name: "ITOCHU" },
  MitsuiItochuIron: { name: "Mitsui-ITOCHU Iron" },
  MitsuiAndCo: { name: "Mitsui & Co." },
  MongoliaGovernment: { name: "Government of Mongolia" },
  MadagascarGovernment: { name: "Government of Madagascar" },
  BlueHorizonInvestments: { name: "Blue Horizon Investments & employees" },
  LabradorIronOreRoyalty: { name: "Labrador Iron Ore Royalty (LIORC)" },
  ChinaBaowuSteelGroup: { name: "China Baowu Steel Group" },
  HancockProspecting: { name: "Hancock Prospecting" },
  NipponSteel: { name: "Nippon Steel" },
  ValterraPlatinum: { name: "Valterra Platinum" },
  AfricanRainbowMinerals: { name: "African Rainbow Minerals" },
  ModikwaCommunities: { name: "Modikwa Communities" },
  SumitomoMetalMiningArizona: { name: "Sumitomo Metal Mining Arizona Inc." },
  SMMMorenci: { name: "SMM Morenci Inc." },
  SMMCerroVerde: {
    name: "Sumitomo Metal Mining (SMM) - Cerro Verde Netherlands B.V.",
  },
  Buenaventura: { name: "Compañía de Minas Buenaventura S.A.A." },
  PublicShareholdersLima: { name: "Public shareholders (Lima SE)" },
  Codelco: { name: "Codelco" },
  MINDIT: { name: "MIND ID & PT Indonesia Papua Metal Dan Mineral" },
  Asarco: { name: "ASARCO" },
  CapstoneCopper: { name: "Capstone Copper" },
  MitsubishiMaterials: { name: "Mitsubishi Materials Corporation" },
  SouthernCopper: { name: "Southern Copper" },
  AgnicoEagle: { name: "Agnico Eagle" },
  Maaden: { name: "Ma'aden" },
  BarrickGold: { name: "Barrick Gold" },
  SABIC: { name: "SABIC" },
  Alcoa: { name: "Alcoa (AWA Saudi Limited)" },
  Teck: { name: "Teck Resources" },
  Newmont: { name: "Newmont" },
  GhanaGovernment: { name: "Government of Ghana" },
  StaatsolieSurinames: { name: "Staatsolie (Suriname)" },
  MaliGovernment: { name: "Government of Mali" },
  AngloGoldAshanti: { name: "AngloGold Ashanti" },
  Sokimo: { name: "SOKIMO (DRC state)" },
  ShandongGold: { name: "Shandong Gold" },
  TanzaniaGovernment: { name: "Government of Tanzania" },
  Fortescue: { name: "Fortescue" },
  FormosaSteel: { name: "Formosa Steel IB Pty Ltd" },
  Cameco: { name: "Cameco" },
  Orano: { name: "Orano" },
  TEPCOResources: { name: "TEPCO Resources" },
  Kazatomprom: { name: "Kazatomprom" },
  BEEPartners: { name: "BEE Partners" },
  GoldRoad: { name: "Gold Road Resources" },
  MinorityShareholders: { name: "Minority Shareholders" }
} as const satisfies Record<string, Company>;

export type CompanyKey = keyof typeof Companies;
export const companiesList: Company[] = Object.values(Companies);
